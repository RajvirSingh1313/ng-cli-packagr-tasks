...
module.exports = function (ctx: NgPackagerHooksContext) {
  async function writePackageTransformer(taskContext: EntryPointTaskContext) {
    ...
    const apiPackage = getApiPackage(
      publicApiFilePath,
      tsConfigOptions,
      logger
    );

    const apiExtractorFilePath = Path.join(
      Path.dirname(publicApiFilePath),
      "api-extractor.json"
    );
...
export function getApiPackage(
  ...
  const parsedCommandLine: ts.ParsedCommandLine = ts.parseJsonConfigFileContent(
    {
      ...tsConfigJson,
      compilerOptions: { ...TS_DEFAULT_CONFIG_OPTIONS, ...compilerOptions },
    },
    ts.sys,
    process.cwd()
  );
  ...
  const collector: Collector = new Collector({
    ...
    entryPointFile:
      Path.isAbsolute(entryPoint) || !rootDir
        ? entryPoint
